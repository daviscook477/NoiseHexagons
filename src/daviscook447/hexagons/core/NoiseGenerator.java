...
public class NoiseGenerator {
	...
	public NoiseGenerator() {
		noise = new SimplexNoiseOctave((int) System.currentTimeMillis());
	}
	
	public NoiseGenerator(int seed) {
		noise = new SimplexNoiseOctave(seed);
		
	}
	
	// generate octave noise by adding multiple 'octaves' of noise together
	public float octaveNoiseAt(float x, float y, int octaves) {
		float result = 0;
		float frequencyCount = 0;
		for (int i = 0; i < octaves; i++) {
			float frequency = (float) Math.pow(2, i);
			frequencyCount += frequency;
			result += noiseAt(x*frequency,y*frequency)*frequency;
		}
		return result / frequencyCount;
	}
	
	// generate octave noise over time
	public float octaveNoiseAtTime(float x, float y, int octaves, float time) {
		float result = 0;
		float frequencyCount = 0;
		for (int i = 0; i < octaves; i++) {
			float frequency = (float) Math.pow(2, i);
			frequencyCount += frequency;
			result += noiseAtTime(x*frequency,y*frequency,time)*frequency;
		}
		return result / frequencyCount;
	}
	
	// generate noise using the simplex noise generator
	public float noiseAt(float x, float y) {
		return (float) noise.noise(x, y);
	}
	
	public float noiseAtTime(float x, float y, float t) {
		return (float) noise.noise(x, y, t);
	}

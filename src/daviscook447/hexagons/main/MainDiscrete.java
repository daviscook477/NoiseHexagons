...
// Makes the greyscale more discrete, as in, it can only take on X different values instead of 255 e.g. only 0, 32, 64, 96, etc...
public class MainDiscrete {
	...
	public static final String DEFAULT_OUT_FILE_PATH = "output/perlin_noise_hexagons_discrete." + IMAGE_FORMAT;
	
	private static final float ROOT_3_OVER_2 = (float) Math.sqrt(3.0) * 0.5f;
	private static final int DISCRETENESS = 32;
	...
	public static Color floatToColor(float value) {
		...
		return new Color(valueScaled, valueScaled, valueScaled); // Shades of grey
	}
	...
	public static void applyNoiseToHexagonGrid(NoiseGenerator png, Hexagon[][] hexGrid) {
		for (int i = 0; i < hexGrid.length; i++) {
			for (int j = 0; j < hexGrid[i].length; j++) {
				...
				// update the hexes color by converting the noise to a greyscale color
